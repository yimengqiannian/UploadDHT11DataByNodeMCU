...
local function safeAdd (x, y)
	...
	local lsw = band(x , 0xffff) + band(y , 0xffff)
	local msw = arshift(x , 16) + arshift(y , 16) + arshift(lsw , 16)
	return bor( lshift(msw , 16) , band(lsw , 0xffff) )
end
...
local function bitRotateLeft (num, cnt)
	return bor( lshift(num , cnt) , rshift(num , (32 - cnt)) )
end
...
local function md5ff (a, b, c, d, x, s, t)
	return md5cmn( bor(band(b , c) , band(bnot(b) , d) ), a, b, x, s, t)
end
local function md5gg (a, b, c, d, x, s, t)
	return md5cmn( bor(band(b , d) , band(c , bnot(d)) ), a, b, x, s, t)
end
local function md5hh (a, b, c, d, x, s, t)
	return md5cmn( bxor(b , c , d), a, b, x, s, t)
end
local function md5ii (a, b, c, d, x, s, t)
	return md5cmn( bxor(c , bor( b , bnot(d) ) ), a, b, x, s, t)
end
...
local function binlMD5 (x, len)
	--append padding
	x[1+arshift(len , 5)] = bor( x[1+arshift(len , 5)] , lshift(0x80 , (len % 32)) )
	x[1+lshift(rshift( len + 64 , 9 ) , 4) + 14] = len
...
local function binl2rstr (input)
	...
	for i = 0,length32-1,8 do
	  table.insert(output,string.char( band(rshift( input[1+ arshift(i , 5)] , i % 32 ) , 0xff ) ) )
	end
...
local function rstr2binl (input)
	local output = {}
	for i = 1,arshift( string.len(input) , 2) do
	  output[i] = 0
	end
	local length8 = string.len(input) * 8
	for i = 0,length8-1,8 do
	  local p = 1+arshift(i , 5);
	  if output[p] == nil then output[p] = 0 end
	  output[p] =  bor( output[p], lshift( band(input:byte((i / 8)+1) , 0xff) , (i % 32) ) )
	end
	return output
end
...
local function rstrHMACMD5 (key, data)
	...
	for i = 1,16 do
	  ipad[i] = bxor(bkey[i] , 0x36363636)
	  opad[i] = bxor(bkey[i] , 0x5c5c5c5c)
	end
...
local function rstr2hex (input)
	...
	for i = 1,string.len(input) do
	  ...
	  table.insert(output , charAt(hexTab, 1+band( rshift(x , 4) , 0x0f)) )
	  table.insert(output , charAt(hexTab, 1+band(x , 0x0f)) )
	end
	return table.concat(output,'')
end
